...
} = require("../../services/sozoXpressPayoutService");

exports.initiateXpressPayout = async (req, res, next) => {
  try {
    ...
    if (amount < 1000) {
      return res.status(400).json({
        success: false,
        message: "Minimum payout amount is 1000 rupees",
      });
    }
    ...
    const [user, bank, kyc] = await Promise.all([
      ...
      SozoXpressPayoutBank.findOne({
        ...
        .select(
          "payoutBankId bankName accountHolderName accountNumber ifscCode",
        )
        .lean(),
    ...
    const response = await initiateXpressPayoutTransfer({
      userId,
