...
exports.getCommissionReportStats = async (req, res, next) => {
  try {
    let {
      page = 1,
      limit = 10,
    ...
    const formattedData = result
      ? {
          ...
          grossCommission: paiseToRupee(result?.grossCommission),
          totalTds: paiseToRupee(result?.totalTds),
          netCommission: paiseToRupee(result?.netCommission),
        }
      : null;

    return res.status(200).json({
      success: true,
      message: "Commission Report Stats",
      data: formattedData,
    });
  } catch (error) {
    next(error);
...
exports.completeCommissionReport = async (req, res, next) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: "Commission Report Found",
      data: formattedData,
