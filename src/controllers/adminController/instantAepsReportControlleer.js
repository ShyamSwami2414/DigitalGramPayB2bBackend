...
const getAepsStats = async (req, res, next) => {
  try {
    ...
    if (!user) {
      ...
      [result] = await InstantAepsReport.aggregate(pipeline);
    }
...
//     const [result] = await InstantAepsReport.aggregate(pipeline);
...
//       ? {
//           total: {
//             count: result.totalCount ?? result.total?.count ?? 0,
//             amount: paiseToRupee(
//               result.totalAmount ?? result.total?.amount ?? 0,
//             ),
//             commission: paiseToRupee(
//               result.totalCommission ?? result.total?.commission ?? 0,
//             ),
//           },
//           success: {
//             count: result.successCount ?? result.success?.count ?? 0,
//             amount: paiseToRupee(
//               result.successAmount ?? result.success?.amount ?? 0,
//             ),
//           },
//           pending: {
//             count: result.pendingCount ?? result.pending?.count ?? 0,
//             amount: paiseToRupee(
//               result.pendingAmount ?? result.pending?.amount ?? 0,
//             ),
//           },
//           failed: {
//             count: result.failedCount ?? result.failed?.count ?? 0,
//             amount: paiseToRupee(
//               result.failedAmount ?? result.failed?.amount ?? 0,
//             ),
//           },
//         }
//       : defaultStats;
...
const getCompleteAepsReport = async (req, res, next) => {
  try {
    ...
    const aepsReport = await InstantAepsReport.aggregate([
      {
        $match: reportMatch,
      },
...
const getAepsReportById = async (req, res, next) => {
  try {
    ...
    const [report] = await InstantAepsReport.aggregate([
      {
        $match: {
