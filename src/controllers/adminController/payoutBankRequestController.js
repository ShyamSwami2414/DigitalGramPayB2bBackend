...
exports.payoutBankRequests = async (req, res, next) => {
    try {
...
exports.approveRejectPayoutBankRequest = async (req, res, next) => {
    try {
        const { id } = req.params;
        let { status } = req.body;
        ...
        return res.status(200).json({
            success: true,
            message: status === "approved" ? "Payout bank request approved" : "Payout bank request rejected",
